...
class PatchEmbed2D(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None, **kwargs):
        ...
        self.stride = [patch_size, patch_size] if isinstance(patch_size, int) else patch_size
    ...
    def compute_conv_feature_map_size(self, input_size):
        return np.prod([self.output_dim, *[i//j for i,j in zip(input_size, self.stride)]], dtype=np.int64)
...
class SS2D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.d_conv = d_conv if isinstance(d_conv, (list, tuple)) else [d_conv, d_conv]
        self.expand = expand
    ...
    def forward_core(self, x: torch.Tensor):
        ...
        xs = torch.cat([x_hwwh, torch.flip(x_hwwh, dims=[-1])], dim=1) # (b, k, d, l) (B, K, d_inner, L)
...
def load_pretrained_ckpt(
    model, 
    ckpt_path=join(pretrained_models, "vmamba_tiny_e292.pth")
):
